    ...
    if(newstate == PLAYER_STATE_DRIVER)
    {
        ...
        if(Vehicle_Job(vehicleid) == JOB_TRUCKER)
        {
            ...
            else
            {
                ...
                {
                    format(line, sizeof(line), "{DADADA}%s\t{DADADA}%i caja%s\t{64A752}$%i{DADADA}-{64A752}$%i\n", g_rgeTruckerRoutes[i][e_szRouteName], g_rgeTruckerRoutes[i][e_iBoxCount], (g_rgeTruckerRoutes[i][e_iBoxCount] > 1 ? "s" : ""), g_rgeTruckerRoutes[i][e_iPaymentMin], g_rgeTruckerRoutes[i][e_iPaymentMax]);
...
public OnPlayerEnterDynamicCP(playerid, checkpointid)
{
    ...
    { 
        ...
        else if(checkpointid == g_iTruckerCentralCp)
        {
            ...
            new pay = math_random(g_rgeTruckerRoutes[route][e_iPaymentMin], g_rgeTruckerRoutes[route][e_iPaymentMax]);
